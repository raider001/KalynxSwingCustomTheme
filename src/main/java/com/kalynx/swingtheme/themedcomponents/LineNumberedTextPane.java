...
public class LineNumberedTextPane extends ThemedPanel {
    ...
    private transient final Map<Integer, List<CommentAnnotation>> lineComments = new HashMap<>();
    ...
    private void showCommentTooltip(Point point, int lineNumber) {
        List<CommentAnnotation> comments = lineComments.get(lineNumber);
    ...
    public void setComments(List<? extends CommentAnnotation> comments) {
        lineComments.clear();
        for (CommentAnnotation comment : comments) {
    ...
    public List<CommentAnnotation> getCommentsForLine(int lineNumber) {
    ...
    public class LineNumberPanel extends JPanel {
        ...
        private void drawLineBackground(Graphics2D g2d, int lineNumber, int y, int lineHeight) {
            ...
            if (hasComments(lineNumber)) {
                List<CommentAnnotation> comments = getCommentsForLine(lineNumber);
                ...
                boolean hasAnyNeedingResolution = comments.stream().anyMatch(CommentAnnotation::needsResolution);
