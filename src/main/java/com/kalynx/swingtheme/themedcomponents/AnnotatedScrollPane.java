...
public class AnnotatedScrollPane extends ThemedScrollPane {
    ...
    private List<CommentAnnotation> cachedComments = List.of();
     ...
     */
    public void setCommentAnnotations(List<? extends CommentAnnotation> comments) {
    ...
    private void pushCommentAnnotations() {
        Map<Integer, List<CommentAnnotation>> byLine = new HashMap<>();
        for (CommentAnnotation comment : cachedComments) {
        ...
        for (Map.Entry<Integer, List<CommentAnnotation>> entry : byLine.entrySet()) {
    ...
    private AnnotatedScrollBar.CommentIndicatorType classifyLine(List<CommentAnnotation> comments) {
        boolean anyNeedsResolution = comments.stream().anyMatch(CommentAnnotation::needsResolution);
        if (!anyNeedsResolution) {
            return AnnotatedScrollBar.CommentIndicatorType.OBSERVATION;
        }
        boolean allResolved = comments.stream()
                .filter(CommentAnnotation::needsResolution)
                .allMatch(CommentAnnotation::isResolved);
        return allResolved
